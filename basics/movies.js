...
// var filimNames=filims.map(f=>f.title)
// console.log(filimNames);

// forEach()

filims.forEach(f=>console.log(f.title))

// movies with genre as action


filims.filter(f=>f.genres.includes("Action")).forEach(f=>console.log(f.title))

// movies with genre as action or drama

filims.filter(f=> f.genres.includes("Action") || f.genres.includes("Drama")).
      forEach(f=>console.log(f.title))
...
var highestRuntime=filims.reduce((f1,f2)=> +f1.runtime > +f2.runtime ? f1 : f2)

// reduce return only sigleObject

console.log(highestRuntime.title);


// lowestRuntime

var smallestRuntimeMOvie=filims.reduce((f1,f2)=> +f1.runtime < +f2.runtime ? f1 : f2)

console.log(smallestRuntimeMOvie.title);


// year in which most number of movies released

var allYears=filims.map(f=>f.year)
var wc={}

allYears.map(y=> y in wc ? wc[y]+=1 : wc[y]=1)

// wc is a object {2000:2}

// object => array

// var data=Object.entries(wc).sort((o1,o2)=> o2[1] - o1[1])

// console.log(data);

// var mostMovieRealeasedYear= Object.entries(wc).reduce((o1,o2)=> o1[1] > o2[1] ? o1 : o2)
// console.log(mostMovieRealeasedYear);



// fetch all directors list
// var allDirectors= filims.map(f=> f.director).map(d=>d.split(",").flat()).flat()

// var dc={}

// allDirectors.map(d=> d in dc ? dc[d]+=1 : dc[d]=1)

// var data=Object.entries(dc).sort((d1,d2)=>d2[1]-d1[1])
// console.log(data);



// var genreArray=filims.map(f=>f.genres).flat()

// var gc={}

// genreArray.map(g=> g in gc ? gc[g]+=1 : gc[g]=1)

// var data=Object.entries(gc).sort((o1,o2)=> o2[1]-o1[1]).slice(0,6)
...
var allDirectors=filims.map(f=>f.director.split(",")).flat()
